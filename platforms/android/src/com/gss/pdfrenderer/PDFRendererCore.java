...
//	private native void replyToAlertInternal(MuPDFAlertInternal alert);
	...
	public PDFRendererCore(Context context, String filename) throws Exception {
...
//			throw new Exception(String.format(context.getString(R.string.cannot_open_file_Path), filename));
	...
	public PDFRendererCore(String filename) throws Exception {
		globals = openFile(filename);
		if (globals == 0) {
			throw new Exception(String.format("Cannot open file: %1$s", filename));
		}
	...
	public PDFRendererCore(Context context, byte buffer[], String magic) throws Exception {
		fileBuffer = buffer;
		globals = openBuffer(magic != null ? magic : "");
		if (globals == 0) {
//			throw new Exception(context.getString(R.string.cannot_open_buffer));
			throw new Exception("Cann't open buffer");
		}
		file_format = fileFormatInternal();
		isUnencryptedPDF = isUnencryptedPDFInternal();
	...
	public PDFRendererCore(byte buffer[], String magic) throws Exception {
		fileBuffer = buffer;
		globals = openBuffer(magic != null ? magic : "");
		if (globals == 0) {
			throw new Exception("Cann't open buffer");
